...
class Fortigate:
    ...
    def AddUserLocal(self, name, passwd, type_user='password', status='enable', email_to='', ldap_server='',
                     radius_server=''):
        """
    ...
    def AddUserLocalIdempotent(self, name, passwd, type_user='password', status='enable', email_to='', ldap_server='',
                               radius_server=''):
        """
    ...
    def SetUserLocal(self, name, passwd, type_user='password', status='enable', email_to='', ldap_server='',
                     radius_server=''):
        """
    ...
    def AddFwPolicyIdempotent(self, srcintf='any', dstintf='any', srcaddr='all', dstaddr='all', service='ALL',
        ...
        service = str(service)
        action = str(action)
        objects = [['srcintf', srcintf], ['dstintf', dstintf], ['srcaddr', srcaddr], ['dstaddr', dstaddr],
                   ['service', service], ['action', action], ['schedule', schedule], ['nat', nat],
                   ['poolname', poolname], ['ippool', ippool], ['status', status], ['traffic-shaper', traffic_shaper],
                   ['traffic-shaper-reverse', traffic_shaper_reverse]]
        if not (self.Exists('cmdb/firewall/policy/', objects)):
            # object does not exist, create it
            # print 'AddFwPolicyIdempotent: object does not exists'
            return self.AddFwPolicy(srcintf, dstintf, srcaddr, dstaddr, service, action, schedule, nat, poolname,
                                    ippool, status, comments, traffic_shaper, traffic_shaper_reverse)
        else:
            # object already Exists
            # print 'AddFwPolicyIdempotent: object already exists'
            return 200

    def SetFwPolicy(self, id, srcintf='any', dstintf='any', srcaddr='all', dstaddr='all', service='ALL',
                    action='accept', schedule='always', nat='disable', poolname='[]', ippool='disable', status='enable',
                    comments='', traffic_shaper='', traffic_shaper_reverse=''):
        """
        Modify a fw policy.
    ...
    def SearchFwPolicyID(self, srcintf='', dstintf='', srcaddr='', dstaddr='', service='', action='', schedule='',
                         nat='', poolname='[]', ippool='', status='', comments='', traffic_shaper='',
    ...
    def AddFwService(self, name, tcp_portrange='', udp_portrange='', protocol='TCP/UDP/SCTP', fqdn='',
                     iprange='0.0.0.0', comment=''):
        '''
    ...
    def AddFwServiceIdempotent(self, name, tcp_portrange='', udp_portrange='', protocol='TCP/UDP/SCTP', fqdn='',
                               iprange='0.0.0.0', comment=''):
        '''
        ...
        if return_code != 200:
            # creation failed, check to see if the object already exists
            objects = [['name', name], ['tcp-portrange', tcp_portrange], ['udp-portrange', udp_portrange],
                       ['protocol', protocol], ['fqdn', fqdn], ['iprange', iprange]]
            if self.Exists('cmdb/firewall.service/custom/', objects):
                return_code = 200
        return return_code

    def SetFwService(self, name, tcp_portrange='', udp_portrange='', protocol='TCP/UDP/SCTP', fqdn='',
                     iprange='0.0.0.0', comment=''):
        '''
    ...
    def AddTrafficShaper(self, name, per_policy, priority, guaranteed_bandwidth, maximum_bandwidth, diffserv='disable',
                         diffservcode='000000'):
        """
    ...
    def AddTrafficShaperIdempotent(self, name, per_policy, priority, guaranteed_bandwidth, maximum_bandwidth,
                                   diffserv='disable', diffservcode='000000'):
        """
        ...
        Http status code: 200 if ok, 4xx if an error occurs
        """
        return_code = self.AddTrafficShaper(name, per_policy, priority, guaranteed_bandwidth, maximum_bandwidth,
                                            diffserv, diffservcode)
        if return_code != 200:
            # creation failed, check to see if the object already exists
            objects = [['name', name]]
    ...
    def AddFwVIP(self, name, extip, extintf, mappedip, portforward='disable', protocol='', extport='0-65535',
                 mappedport='0-65535', comment=''):
        """
        Create vip address.
    ...
    def AddFwVIPidempotent(self, name, extip, extintf, mappedip, portforward='disable', extport='0-65535',
                           mappedport='0-65535', comment=''):
        """
    ...
    def SetFwVIP(self, name, extip, extintf, mappedip, portforward='disable', protocol='', extport='0-65535',
                 mappedport='0-65535', comment=''):
        """
        Modify vip address.
    ...
    def AddFwIPpool(self, name, startip, endip, type_pool='overload', internal_startip='0.0.0.0',
    ...
    def AddFwIPpoolIdempotent(self, name, startip, endip, type_pool='overload', internal_startip='0.0.0.0',
                              internal_endip='0.0.0.0', arp_reply='enable', block_size='128', num_blocks_per_user='8',
                              comment=''):
        """
        ...
        return_code = self.AddFwIPpool(name, startip, endip, type_pool, internal_startip, internal_endip, arp_reply,
                                       block_size, num_blocks_per_user, comment)
        if return_code != 200:
            # creation failed, check to see if the object already exists
            objects = [['name', name]]
    ...
    def AddVPNipsecPhase1(self, name, interface, remote_gw, nattraversal, dpd, psk, ike_version, mode, proposal, dhgrp,
                          keylife=28800, localid=''):
        """
    ...
    def AddVPNipsecPhase1Idempotent(self, name, interface, remote_gw, nattraversal, dpd, psk, ike_version, mode,
                                    proposal, dhgrp, keylife=28800, localid=''):
        """
        ...
        if not (self.Exists('cmdb/vpn.ipsec/phase1-interface/', objects)):
            # object does not exist, create it
            return self.AddVPNipsecPhase1(name, interface, remote_gw, nattraversal, dpd, psk, ike_version, mode,
                                          proposal, dhgrp, keylife, localid)
        else:
            # object already Exists
            return 200

    def AddVPNipsecPhase2(self, name, phase1name, local_addr_type, local_subnet, remote_addr_type, remote_subnet,
                          proposal, pfs, dhgrp, replay, keepalive, keylife_type, keylifeseconds):
        """
    ...
    def AddVPNipsecPhase2Idempotent(self, name, phase1name, local_addr_type, local_subnet, remote_addr_type,
                                    remote_subnet, proposal, pfs, dhgrp, replay, keepalive, keylife_type,
                                    keylifeseconds):
        """
        Create vpn ipsec tunnel phase2.
        ...
        if not (self.Exists('cmdb/vpn.ipsec/phase2-interface/', objects)):
            # object does not exist, create it
            return self.AddVPNipsecPhase2(name, phase1name, local_addr_type, local_subnet, remote_addr_type,
                                          remote_subnet, proposal, pfs, dhgrp, replay, keepalive, keylife_type,
                                          keylifeseconds)
        else:
            # object already Exists
